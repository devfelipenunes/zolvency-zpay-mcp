...
# -----------------------------------------------------------------------------
# 2. FastMCP Instance Initialization
# -----------------------------------------------------------------------------
...
def ensure_agent_identity():
    ...
    if settings.agent_identity_pk and settings.agent_identity_sk:
        _AGENT_IDENTITY_CACHE = (settings.agent_identity_pk, settings.agent_identity_sk)
        return _AGENT_IDENTITY_CACHE

    # Priority 2: Standard Config Directory
    identity_path = settings.config_dir / "agent_identity.json"
    crypt_key = get_encryption_key()
        ...
        try:
            ...
            if crypt_key:
                try:
                    ...
                    # Fallback to plaintext if decryption fails (might be an old unencrypted file)
                    data = json.loads(raw_data)
            else:
                data = json.loads(raw_data)

            if "public_key" in data and "secret" in data:
                logger.info(f"Loaded Persistent Agent Identity: {data['public_key']}")
                _AGENT_IDENTITY_CACHE = (data["public_key"], data["secret"])
                return _AGENT_IDENTITY_CACHE
        except Exception as e:
            logger.warning(f"Failed to read identity from {identity_path}: {e}")

    # Fallback: Generate New Identity
    kp = Keypair.random()
    logger.info(f"Generated New Agent Identity: {kp.public_key}")
    _AGENT_IDENTITY_CACHE = (kp.public_key, kp.secret)
    
    try:
        identity_data = json.dumps({"public_key": kp.public_key, "secret": kp.secret}).encode()
        if crypt_key:
            fernet = Fernet(crypt_key)
            identity_data = fernet.encrypt(identity_data)
...
async def execute_sovereign_payment(
    ...
    allowance = await kit.get_token_allowance(user_anchor_address, CONTRACT_GATEWAY, token)
    if allowance < amount:
        return f"⚠️ **Allowance Error:** Gateway needs permission to spend {amount} XLM. Current allowance: {allowance} XLM."
...
async def poll_sovereign_handshake(session_id: str) -> str:
    ...
    if api_data.get("status") == "COMPLETED":
        # If API says completed but not on blockchain yet, it might be a race condition/ledger delay
        return "⏳ Authorization detected on server, waiting for ledger finalization..."
...
async def get_transaction_report(transaction_hash: str) -> str:
    ...
    h_url = "https://horizon-testnet.stellar.org" if STELLAR_NETWORK == "TESTNET" else "https://horizon.stellar.org"
    async with httpx.AsyncClient() as client:
